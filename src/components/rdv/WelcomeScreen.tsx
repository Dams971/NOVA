...
import { FaCalendarPlus, FaExclamationTriangle, FaCalendarAlt, FaInfoCircle, FaPhone, FaEnvelope, FaClock } from 'react-icons/fa';
...
const WelcomeScreen: React.FC<WelcomeScreenProps> = ({
  ...
  const handleActionClick = (action: WelcomeAction) => {
    ...
    onActionSelect?.(action.action, action.data);
  };

  const getActionIcon = (actionType: string) => {
    const iconMap = {
      start_booking: FaCalendarPlus,
      emergency: FaExclamationTriangle,
      view_calendar: FaCalendarAlt,
      clinic_info: FaInfoCircle
    };
    return iconMap[actionType as keyof typeof iconMap] || FaInfoCircle;
  };
  ...
  return (
    <div className={`max-w-md mx-auto p-6 bg-white rounded-lg shadow-lg ${className}`}>
      ...
      <div className="text-center mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-800 mb-2">NOVA RDV</h1>
        <p className="text-gray-600 text-sm leading-relaxed" role="banner">
          {message}
        </p>
      </div>
