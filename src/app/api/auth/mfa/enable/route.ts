...
async function handleEnableMFA(request: AuthenticatedRequest): Promise<NextResponse> {
  try {
    ...
    // Enable MFA
    await mfaService.enableMFA(user.userId, body.token);
...
export const POST = withCORS(withAuth(handleEnableMFA));