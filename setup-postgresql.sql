...
-- Execute this script in PostgreSQL to create the database and schema

-- Create database (run this as postgres superuser first)
-- CREATE DATABASE nova_db WITH ENCODING 'UTF8' LC_COLLATE='en_US.UTF-8' LC_CTYPE='en_US.UTF-8';

-- Connect to nova_db database and run the rest
\c nova_db;
...
-- Cabinets table (dental practices)
...
-- Practitioners table (dentists, doctors)
...
-- Cabinet users (practitioners and staff associated with cabinets)
...
-- Services table (types of treatments)
...
-- Practitioner services (which services each practitioner offers)
...
CREATE TABLE appointments (
    ...
    -- Contact info (for non-registered patients)
...
-- Success message
SELECT 'NOVA PostgreSQL database schema created successfully!' as message;